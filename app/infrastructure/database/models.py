...
from app.domain.entities.ticket import TicketCategory, TicketPriority, TicketStatus
...
class UserModel(Base):
    ...
    password_hash = Column(String(255), nullable=True)  # Unused; auth by email only
    role = Column(
    ...
    created_tickets = relationship(
        ...
        foreign_keys="TicketModel.created_by",
    )
...
class TicketModel(Base):
    ...
    creator = relationship(
        "UserModel", foreign_keys=[created_by], back_populates="created_tickets"
    )
    assignee = relationship(
        "UserModel", foreign_keys=[assigned_to], back_populates="assigned_tickets"
    )
...
class InventoryModel(Base):
    ...
    status = Column(
        ...
        nullable=False,
        default=InventoryStatus.WORKING,
    )
...
class TodoModel(Base):
    ...
    comments = relationship(
        ...
        back_populates="todo",
        cascade="all, delete-orphan",
        order_by="TodoCommentModel.created_at",
    )
    todo_list_items = relationship(
        "TodoListItemModel",
        back_populates="todo",
        cascade="all, delete-orphan",
    ...
    attachments = relationship(
        ...
        back_populates="todo",
        cascade="all, delete-orphan",
        order_by="TodoAttachmentModel.created_at",
    )
    assigned_users = relationship(
        "UserModel", secondary="todo_assignments", back_populates="assigned_todos"
    )
...
class TodoColumnModel(Base):
    ...
    )  # Frontend column ID (not unique, as each user has their own)
    title = Column(String(255), nullable=False)
    status = Column(
        String(100), nullable=False, index=True
    )  # todo, in_progress, done, or custom
    ...
    user_id = Column(
        ...
        ),  # VARCHAR(36) - UUID stored as string
        ForeignKey("users.id", ondelete="CASCADE"),
...
from app.infrastructure.telegram.models import TelegramLinkTokenModel, UserTelegramModel
